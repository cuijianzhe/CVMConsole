  ...
  <div class="network-page">
        ...
        <div v-show="activeTab === 'overview'" class="tab-pane">
          ...
          <div class="info-card table-card">
            ...
            <el-table :data="bridges" border style="width: 100%;" max-height="280">
              ...
              <el-table-column label="DHCP 地址池" min-width="260">
                <template #default="{ row }">
                  <div v-if="row.dhcp_cidr">
                    <div class="code" style="font-size: 12px;">{{ row.dhcp_cidr }}</div>
                    <div class="code" style="font-size: 12px; color: var(--el-text-color-secondary);">{{ row.dhcp_start }} - {{ row.dhcp_end }}</div>
                  </div>
          ...
          <div class="data-table-card switch-data-card">
            <el-table :data="paginatedSwitches" border v-loading="loading" style="width: 100%;" row-key="id">
              ...
              <el-table-column label="子网" min-width="160">
                <template #default="{ row }">
                  <span class="code">
                    <template v-if="row.bridge_mode === 'bridge'">
                      <template v-if="row.bridge_ip_mode === 'preset'">{{ row.cidr || '预设 IP 段' }}</template>
                      <template v-else>上级路由分配</template>
                    </template>
                    <template v-else>{{ row.cidr }}</template>
                  </span>
                </template>
              </el-table-column>
              ...
              <el-card v-for="row in paginatedSwitches" :key="row.id" class="switch-mobile-card" shadow="hover">
                ...
                <div class="switch-card-body">
                  <div class="switch-card-info-row">
                    ...
                    <span class="switch-card-value code">
                      <template v-if="row.bridge_mode === 'bridge'">
                        <template v-if="row.bridge_ip_mode === 'preset'">{{ row.cidr || '预设 IP 段' }}</template>
                        <template v-else>上级路由分配</template>
                      </template>
    ...
    >
      <el-form :model="switchForm" label-width="140px" label-position="left">
        ...
        <el-form-item v-if="isAdmin" label="目标网桥">
          ...
          <div v-if="selectedSwitchBridge?.mode === 'bridge'" class="form-hint">桥接直通模式下，VM 直接接入物理网络。</div>
        </el-form-item>
        ...
        <el-form-item v-if="selectedSwitchBridge?.mode === 'bridge'" label="IP 分配模式">
          <el-radio-group v-model="switchForm.bridge_ip_mode">
            <el-radio value="upstream">上级路由分配</el-radio>
            <el-radio value="preset" :disabled="!selectedSwitchBridge?.dhcp_cidr">预设 IP 段分配</el-radio>
          </el-radio-group>
          <div v-if="switchForm.bridge_ip_mode === 'upstream'" class="form-hint">由上级路由器的 DHCP 服务分配 IP，不启用内部 DHCP、NAT、安全组和端口转发。</div>
          <div v-else-if="switchForm.bridge_ip_mode === 'preset'" class="form-hint">使用网桥预设的 IP 段分配，由系统 DHCP 服务分配 IP。</div>
          <div v-if="selectedSwitchBridge?.dhcp_cidr && switchForm.bridge_ip_mode === 'preset'" class="form-hint" style="color: var(--el-color-success);">
            预设网段：{{ selectedSwitchBridge.dhcp_cidr }}（{{ selectedSwitchBridge.dhcp_start }} - {{ selectedSwitchBridge.dhcp_end }}），网关：{{ selectedSwitchBridge.dhcp_gateway }}，DNS：{{ selectedSwitchBridge.dhcp_dns }}
          </div>
          <div v-if="!selectedSwitchBridge?.dhcp_cidr" class="form-hint" style="color: var(--el-color-warning);">目标网桥未配置 DHCP 地址池，请先在网桥设置中配置。</div>
        </el-form-item>
    ...
    >
      ...
      <el-form :model="bridgeForm" label-width="130px" label-position="left">
        ...
        <el-divider content-position="left">DHCP 地址池（可选）</el-divider>
        <el-form-item label="网段(CIDR)">
          <el-input v-model="bridgeForm.dhcp_cidr" placeholder="如 192.168.1.0/24" />
          <div class="form-hint">配置后将在网桥上启动 DHCP 服务，VM 可自动获取该网段 IP。</div>
        </el-form-item>
        <el-form-item label="起始 IP">
          <el-input v-model="bridgeForm.dhcp_start" placeholder="如 192.168.1.100" />
          <div class="form-hint">留空时自动计算为网关之后的第一个可用 IP。</div>
        </el-form-item>
        <el-form-item label="结束 IP">
          <el-input v-model="bridgeForm.dhcp_end" placeholder="如 192.168.1.254" />
          <div class="form-hint">留空时自动计算为网段最后一个可用 IP。</div>
        </el-form-item>
        <el-form-item label="网关地址">
          <el-input v-model="bridgeForm.dhcp_gateway" placeholder="如 192.168.1.1" />
          <div class="form-hint">留空时自动计算为网段内第一个可用 IP。</div>
        </el-form-item>
        <el-form-item label="DNS 服务器">
          <el-input v-model="bridgeForm.dhcp_dns" placeholder="如 223.5.5.5 119.29.29.29" />
          <div class="form-hint">多个 DNS 用空格分隔，留空时使用系统默认值。</div>
        </el-form-item>
      </el-form>
      <template #footer>
...
const switchForm = reactive({ username: '', name: '', bridge_name: 'br-ovs', bridge_ip_mode: 'upstream', bridge_vlan_id: 0, allow_promiscuous: false, allow_mac_change: false, allow_forged_transmits: false, cidr: '', gateway_ip: '', dhcp_start: '', dhcp_end: '', traffic_down_gb: 0, traffic_up_gb: 0, bandwidth_down_mbps: 0, bandwidth_up_mbps: 0, bandwidth_mbps: 0 })

const bridgeDialogVisible = ref(false)
const bridgeForm = reactive({ name: '', mode: 'bridge', uplink_if: '', migrate_host_ip: true, dhcp_cidr: '', dhcp_start: '', dhcp_end: '', dhcp_gateway: '', dhcp_dns: '' })
