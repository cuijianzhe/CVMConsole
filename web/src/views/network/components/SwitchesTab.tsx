...
export default function SwitchesTab({
  ...
  const columns: ColumnProps<VpcSwitch>[] = [
      ...
      : []),
    {
      ...
      dataIndex: 'cidr',
      render: (_text, row) => {
        // 预设 IP 段分配（bridge_ip_mode=preset）也展示网段，避免被误显示为“上级网络分配”
        let label = '纯二层'
        if (row.is_system) label = row.cidr || '系统配置'
        else if (row.dhcp_enabled || row.bridge_ip_mode === 'preset') label = row.cidr || '—'
        else if (row.uplink_if) label = '上级网络分配'
        return <span className="qvm-mono">{label}</span>
      },
    },
    {
      title: '网关',
      ...
      render: (text, row) => {
        const showGateway = row.is_system || row.dhcp_enabled || row.bridge_ip_mode === 'preset'
        return <span className="qvm-mono">{showGateway ? text || '—' : '—'}</span>
      },
    },
    {
      title: '月下行流量',
