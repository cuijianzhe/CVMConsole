...
export const endpointGroups = [
  ...
  },
  {
    ...
    endpoints: [
      ...
      ep('POST', '/vm/import-disk', '管理员通过磁盘路径导入虚拟机', {
        body: 'JSON: name, disk_path(磁盘绝对路径), os_type, os_variant, vcpu, ram, remark, switch_id, security_group_id, storage_pool_id, user_data(可选, cloud-init YAML或PowerShell脚本) 等',
        response: 'data: task_id。导入操作为异步任务，请继续查询任务详情。',
        notes: [admin, elastic, '需指定宿主机上已有磁盘文件的绝对路径。', '仅支持 qcow2 和 raw 格式磁盘。'],
        highRisk: 'import_disk'
      }),
  ...
  },
  {
    ...
    endpoints: [
      ...
      ep('POST', '/self/vm/import', '导入 VM 到自己账号', { body: 'JSON: file/category/path, name, remark, vcpu, ram, switch_id, security_group_id, credentials, user_data(可选, cloud-init YAML或PowerShell脚本) 等', notes: [elastic] }),
  ...
  },
  {
    ...
    endpoints: [
      ...
      // vGPU 管理
      ep('GET', '/host/vgpu/profiles', '获取 vGPU 类型列表', {
        notes: [admin],
        response: 'data: [{id, pci_device, profile_name, description, max_instances, used_instances, memory_mb, available}]。'
      }),
      ep('POST', '/host/vgpu/profiles/discover', '刷新检测 vGPU 设备', {
        notes: [admin],
        response: 'data: 检测到的 vGPU profile 数量。'
      }),
      ep('GET', '/host/vgpu/instances', '获取 vGPU 实例列表', {
        notes: [admin],
        response: 'data: [{id, uuid, profile_id, vm_name, active, profile}]。'
      }),
      ep('POST', '/host/vgpu/instances', '创建 vGPU 实例', {
        body: 'JSON: profile_id(必填)',
        notes: [admin, '创建成功后需要绑定到 VM 才能使用。', '需确保宿主机已启用 vGPU 功能（加载 vfio-pci、nvidia_vgpu_vfio 模块，安装 NVIDIA vGPU Manager）。'],
        highRisk: 'create_vgpu_instance'
      }),
      ep('DELETE', '/host/vgpu/instances/:uuid', '删除 vGPU 实例', {
        pathParams: ['uuid'],
        notes: [admin, '删除前需先从 VM 解绑。'],
        highRisk: 'delete_vgpu_instance'
      }),
      ep('PUT', '/host/vgpu/instances/:id/attach', '绑定 vGPU 到 VM', {
        pathParams: ['id'],
        body: 'JSON: vm_name(必填)',
        notes: [admin, 'VM 必须处于关机状态。'],
        highRisk: 'attach_vgpu_to_vm'
      }),
      ep('PUT', '/host/vgpu/instances/:id/detach', '从 VM 解绑 vGPU', {
        pathParams: ['id'],
        notes: [admin, 'VM 必须处于关机状态。'],
        highRisk: 'detach_vgpu_from_vm'
      })
    ]
  },
  {
