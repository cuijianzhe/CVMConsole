...
const initCharts = () => {
  ...
  diskChart.setOption({
    ...commonChartOptions(undefined, getDiskYAxisFormatter()),
...
const onChartModeChange = (mode) => {
  if (mode === 'realtime') {
    ...
    if (charts[3]) charts[3].setOption({ title: { text: getDiskChartTitle(), textStyle: { fontSize: 14, color: '#8f9399' } } })
    prevNet = { rx: 0, tx: 0 }
    prevDisk = { rd: 0, wr: 0 }
