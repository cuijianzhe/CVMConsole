...
func EnsureVPCSwitchRuntime(sw model.VPCSwitch) error {
	...
	if HookSwitchUsesDirectBridge(sw) {
		...
		if sw.OwnsBridge {
			...
			// 预设 IP 模式：OVS 网桥创建后启动 dnsmasq DHCP 服务
			if sw.BridgeIPMode == BridgeIPModePreset && sw.CIDR != "" && sw.DHCPStart != "" && sw.DHCPEnd != "" && HookStartBridgeDNSMasq != nil {
				...
				if err := HookStartBridgeDNSMasq(spooferBridge); err != nil {
					logger.App.Warn("预设模式启动 DHCP 服务失败", "bridge", bridgeName, "error", err)
				}
			}
		...
		if err := model.DB.Where("name = ?", bridgeName).First(&bridge).Error; err != nil {
			...
			if bridge.DHCPCIDR != "" && HookStartBridgeDNSMasq != nil {
				if err := HookStartBridgeDNSMasq(bridge); err != nil {
					logger.App.Warn("启动桥接网桥 DHCP 服务失败", "bridge", bridge.Name, "error", err)
				}
			} else if sw.BridgeIPMode == BridgeIPModePreset && sw.CIDR != "" && sw.DHCPStart != "" && sw.DHCPEnd != "" && HookStartBridgeDNSMasq != nil {
				// 预设 IP 模式：网桥本身无 DHCP 配置时，使用交换机的 DHCP 配置启动 dnsmasq
				spooferBridge := model.NetworkBridge{
					Name:        bridgeName,
					DHCPCIDR:    sw.CIDR,
					DHCPGateway: sw.GatewayIP,
				...
				if err := HookStartBridgeDNSMasq(spooferBridge); err != nil {
					logger.App.Warn("预设模式启动桥接网桥 DHCP 服务失败", "bridge", bridgeName, "error", err)
				}
			}
		}
		if HookEnsureLocalDNSMasqInputRules != nil {
			if err := HookEnsureLocalDNSMasqInputRules(bridgeName); err != nil {
				logger.App.Warn("配置桥接网桥 DHCP/DNS 入站规则失败", "bridge", bridgeName, "error", err)
			}
		}
		return ApplyVPCSwitchBandwidth(sw)
	}
