...
func GetVncStatus(vmName string) (*VncInfo, error) {
	...
	vmState, err := getDomainStateRPC(vmName)
	if err != nil {
	...
	if vmState != "running" && vmState != "paused" {
		// 从 XML 配置检查是否有 VNC
		xmlStr, xmlErr := getDomainXMLRPC(vmName, libvirt.DomainXMLInactive)
		if xmlErr == nil {
	...
	vncOutput, vncErr := qemuMonitorCommandRPC(vmName, "info vnc", 1)
	if vncErr == nil {
...
func getVMXMLForVnc(vmName, vmState string) (string, error) {
	if vmState == "running" {
		return getDomainXMLRPC(vmName, 0)
	}
	return getDomainXMLRPC(vmName, libvirt.DomainXMLInactive)
}
...
func EnableVnc(vmName, password string) error {
	...
	detectedOSType := detectVMOSType("", xmlStr)
	xmlStr = ApplyVMVideoModelToDomainXML(xmlStr, ParseVMVideoModelFromDomainXML(xmlStr), detectedOSType)
...
func ChangeVncPassword(vmName, newPassword string) error {
	...
	// 热修改密码
	_, err = qemuMonitorCommandRPC(vmName, fmt.Sprintf("set_password vnc %s", newPassword), 1)
	if err != nil {
		return fmt.Errorf("修改密码失败: %w", err)
	}

	// 同步持久化配置
	xmlStr, xmlErr := getDomainXMLRPC(vmName, 0)
	if xmlErr == nil {
		updatedXML := vncPasswdRe.ReplaceAllString(xmlStr, fmt.Sprintf("passwd='%s'", newPassword))
		if _, defineErr := defineDomainXMLRPC(updatedXML); defineErr != nil {
